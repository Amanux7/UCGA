...
\twocolumn[{%
  ...
  \begin{abstract}
  \noindent
  This paper introduces the \textbf{Unified Cognitive Graph Architecture (\UCGA{})}, a novel graph-native cognitive framework designed to address fundamental limitations of traditional neural architectures in reasoning, memory integration, and adaptive intelligence. \UCGA{} has been extended into a full AGI-ready cognitive architecture implementing seven core intelligence layers inspired by cognitive science, neuroscience, and modern AI systems.

  We present the architecture, mathematical formulation, and a fully differentiable PyTorch implementation comprising seven intelligence layers: Perception, Cognitive Graph, Working Memory, Reasoning, Global Workspace, Executive Controller, and Action. Experimental validation on standard benchmarks demonstrates the framework's learning capability: \UCGA{} achieves \textbf{90.5\%} test accuracy on AG News text classification and \textbf{60.0\%} on CIFAR-10 image classification, establishing its viability across both NLP and computer vision domains. A comprehensive test suite of 43 unit and integration tests validates architectural correctness. We further outline a roadmap for scaling \UCGA{} toward full artificial general intelligence capability.
...
\begin{enumerate}[leftmargin=1.4em, itemsep=0.15em]
  \item A \textbf{graph-native cognitive architecture} with seven core intelligence layers modelling distinct cognitive functions.
...
\begin{itemize}[leftmargin=1.4em, itemsep=0.15em]
  \item $\mathcal{V} = \{v_1, \ldots, v_7\}$: Intelligence layers --- Perception, Cognitive Graph, Working Memory, Reasoning, Global Workspace, Executive Controller, and Action.
  \item $\mathcal{E} \subseteq \mathcal{V} \times \mathcal{V}$: Directed edges encoding information flow between cognitive layers.
...
\begin{table}[t]
  ...
  \begin{tabular}{@{}llp{3cm}@{}}
    \toprule
    \textbf{Layer} & \textbf{Role} & \textbf{Mechanism} \\
    \midrule
    Perception          & Multimodal inputs    & Text, image, audio, sensor processing \\
    Cognitive Graph     & Knowledge structure  & Explicit nodes \& persistent memory \\
    Working Memory      & Temporary memory     & Active context \& goals \\
    Reasoning           & Inference engine     & Graph-based \& recursive reasoning \\
    Global Workspace    & Attention            & Broadcast \& coordination system \\
    Executive Controller& Decision making      & Goal management \& planning \\
    Action              & Execution            & Response generation \& tool usage \\
    \bottomrule
  \end{tabular}
\end{table}
...
\UCGA{} operates through a continuous cognitive loop. At each timestep $t$, information flows through the architecture according to the AGI Cognitive Cycle defined in \Cref{alg:cognitive_cycle}.

\begin{algorithm}[t]
\caption{AGI Cognitive Cycle}
\label{alg:cognitive_cycle}
\DontPrintSemicolon
\KwIn{Environment state $\bx$, persistent memory $\bM$, cognitive graph $\mathcal{G}$}
\KwOut{Action sequence $\hat{a}$, updated graph $\mathcal{G}'$}
\BlankLine
\For{$t \gets 1$ \KwTo $T$}{
  $\bv_{\text{perc}} \gets \textsc{Perceive}(\bx)$\;
  $\bv_{\text{wm}} \gets \textsc{UpdateWorkingMemory}(\bv_{\text{perc}})$\;
  $\bv_{\text{graph}} \gets \textsc{ActivateCognitiveGraph}(\bv_{\text{wm}}, \mathcal{G})$\;
  $\bv_{\text{reas}} \gets \textsc{PerformReasoning}(\bv_{\text{graph}})$\;
  $\bv_{\text{goals}} \gets \textsc{GenerateGoals}(\bv_{\text{reas}})$\;
  $\bv_{\text{plan}} \gets \textsc{PlanActions}(\bv_{\text{goals}})$\;
  $\hat{a} \gets \textsc{ExecuteActions}(\bv_{\text{plan}})$\;
  $\mathcal{G}' \gets \textsc{LearnAndUpdateGraph}(\mathcal{G}, \hat{a})$\;
}
\Return{$\hat{a}, \mathcal{G}'$}\;
\end{algorithm}

The action output feeds back as environmental state, and the updated knowledge graph becomes the basis for the next cognitive cycle, enabling the architecture to iteratively learn and adapt.
...
\begin{itemize}[leftmargin=1.4em, itemsep=0.15em]
  \item \textbf{7 core intelligence layers}: Implemented via specialized \texttt{nn.Module} subclasses for each layer (Perception, Working Memory, etc.).
...
\begin{itemize}[leftmargin=1.4em, itemsep=0.1em]
  \item Complete AGI-ready architecture with 7 core intelligence layers
