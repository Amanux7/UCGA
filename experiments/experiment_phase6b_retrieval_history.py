...
def make_model(variant: str, task: TaskData, t_steps: int, state_dim: int) -> UCGAModel:
    ...
    raise ValueError(f"Unknown Phase 6B variant: {variant}")
...
def make_rows(result: Dict) -> Tuple[Dict, List[Dict]]:
    ...
    history_norm = result["history_norm_by_step"]
    history_ratio = result["history_ratio_by_step"]
    ...
    run_row = {
        ...
        "history_norm_mean": finite_mean(history_norm),
        "history_norm_delta": (history_norm[-1] - history_norm[0]) if len(history_norm) > 1 else 0.0,
        "history_ratio_mean": finite_mean(history_ratio),
        ...
        step_rows.append(
            {
                ...
                "history_norm": history_norm[step] if step < len(history_norm) else float("nan"),
                "history_ratio": history_ratio[step] if step < len(history_ratio) else float("nan"),
                "alpha": result["alpha"],
...
def write_report(output_dir: Path, summary_rows: List[Dict], smoke: bool) -> None:
    lines = [
        "# UCGA Phase 6B Retrieval History Accumulation Report",
        "",
        "This experiment compares explicit memory retrieval against mean-pooled retrieval history accumulation.",
        "",
    ]
    if smoke:
        lines.extend(
    ...
    lines.extend(
        [
            ...
            "",
            "| Task | Variant | T | Accuracy | Std | GradNorm | Alpha | Entropy | History Ratio | Runtime |",
            "| --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |",
        ]
    )
    for row in summary_rows:
        lines.append(
            "| {task} | {variant} | {T} | {acc} | {std} | {grad} | {alpha} | {entropy} | {ratio} | {runtime}s |".format(
                ...
                ratio=format_float(row["history_ratio_mean_mean"]),
    ...
    for task in sorted({row["task"] for row in summary_rows}):
        ...
        for variant in sorted({row["variant"] for row in task_rows}):
            ...
            if 1 in rows and 3 in rows and 5 in rows:
                monotonic = (
                    rows[5]["accuracy_mean"] > rows[3]["accuracy_mean"] > rows[1]["accuracy_mean"]
                )
                lines.append(
                    f"- {task} / {variant}: T=5 > T=3 > T=1 is {'observed' if monotonic else 'not observed'}."
                )
                lines.append(
                    f"- {task} / {variant}: T=5 history ratio={format_float(rows[5]['history_ratio_mean_mean'])}, "
                    f"query delta change={format_float(rows[5]['q_delta_delta_mean'])}, "
                    f"slot shift={format_float(rows[5]['slot_shift_mean_mean'])}."
                )
        ...
        if 5 in baseline and 5 in history:
            improvement = history[5]["accuracy_mean"] - baseline[5]["accuracy_mean"]
            lines.append(f"- {task}: history_mean T=5 vs baseline T=5 accuracy delta={improvement:.4f}.")
        lines.append("")

    lines.extend(["## Failure Modes", ""])
    for task in sorted({row["task"] for row in summary_rows}):
        history_rows = [row for row in summary_rows if row["task"] == task and row["variant"] == "history_mean" and row["T"] > 1]
        if not history_rows:
            continue
        ratio = float(np.nanmean([row["history_ratio_mean_mean"] for row in history_rows]))
        q_delta = float(np.nanmean([row["q_delta_delta_mean"] for row in history_rows]))
        slot_shift = float(np.nanmean([row["slot_shift_mean_mean"] for row in history_rows]))
        chain = float(np.nanmean([row["chain_full_accuracy_mean"] for row in history_rows]))
        lines.append(f"- {task}:")
        if ratio < 0.25:
            lines.append("  - A. History is likely ignored: history/current retrieval ratio is low.")
        elif ratio > 2.0:
            lines.append("  - B. History may dominate: ratio is high enough to risk oversmoothing.")
        else:
            lines.append("  - History contribution is present but not automatically useful.")
        if slot_shift < 0.10:
            lines.append("  - C. Retrieval quality remains a bottleneck: attention changes weakly across steps.")
        if chain < 0.20:
            lines.append("  - D. Memory representation or supervision is insufficient for reliable fact chaining.")
        if q_delta >= 0:
            lines.append("  - Query updates do not become progressively smaller on average.")
        lines.append("")
    ...
    lines.extend(
        [
            ...
            "",
            "Move from passive mean history to structured evidence state: use a gated accumulator or GRU that can decide what to retain, and add an auxiliary support-fact alignment diagnostic so retrieval quality and evidence integration can be separated.",
            "",
            "## Outputs",
            "",
            "- `phase6b_comparison_table.csv`: seed-aggregated final comparison table.",
            "- `phase6b_run_metrics.csv`: one row per task/variant/T/seed.",
            "- `phase6b_step_metrics.csv`: per-step entropy, query delta, retrieval norm, history norm, and history ratio.",
            "- `phase6b_raw_results.json`: raw per-run diagnostics excluding full attention tensors.",
        ]
    )
    (output_dir / "phase6b_report.md").write_text("\n".join(lines), encoding="utf-8")


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="UCGA Phase 6B retrieval history accumulation experiment")
    parser.add_argument("--variants", nargs="+", default=["baseline_explicit_memory", "history_mean"])
    ...
    parser.add_argument("--output-dir", type=Path, default=Path("outputs/phase6b"))
...
def main() -> None:
        ...
        for variant in args.variants:
            for t_steps in args.T:
                for seed in range(args.seeds):
                    print(f"Phase 6B | task={task.name} | variant={variant} | T={t_steps} | seed={seed}")
    ...
    write_csv(args.output_dir / "phase6b_run_metrics.csv", run_rows)
    write_csv(args.output_dir / "phase6b_step_metrics.csv", step_rows)
    write_csv(args.output_dir / "phase6b_comparison_table.csv", summary_rows)
    (args.output_dir / "phase6b_raw_results.json").write_text(json.dumps(raw_results, indent=2), encoding="utf-8")
    write_report(args.output_dir, summary_rows, smoke=args.smoke)

    print("\nPhase 6B final comparison")
    for row in summary_rows:
        print(
            f"{row['task']:<24} {row['variant']:<25} T={row['T']} "
            f"acc={row['accuracy_mean']:.4f} entropy={row['entropy_mean_mean']:.4f} "
            f"history_ratio={row['history_ratio_mean_mean']:.4f} runtime={row['runtime_sec_mean']:.2f}s"
        )
